...
跨组件传递局部@Builder函数时，会使用.bind(this)更改函数上下文，但这可能会导致组件的父子关系与状态管理的父子关系不一致。而@LocalBuilder无论是否使用.bind(this)，都不会改变组件的父子关系，即@LocalBuilder中定义组件所属的父组件是确定的，无法被改变。详情请参考[@LocalBuilder和@Builder区别说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-localbuilder#localbuilder和builder区别说明)。
...
@LocalBuilder函数的参数传递有按值传递和按引用传递两种，均需遵守以下规则：
  ...
  * @LocalBuilder内的UI语法遵循[UI语法规则](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-create-custom-components#build函数)。
