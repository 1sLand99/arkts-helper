...
目前分布式数据对象只能在[跨端迁移](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-continuation-guide)和通过跨设备Call调用实现的多端协同场景中使用。
...
#### 约束限制
  ...
  * 目前分布式数据对象只能在[跨端迁移](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-continuation-guide)和通过跨设备Call调用实现的多端协同场景中使用。跨端迁移场景下，每个分布式数据对象大小不超过150KB；多端协同场景下，每个分布式数据对象大小不超过500KB。
...
以下是分布式对象跨设备数据同步功能的相关接口，大部分为异步接口。异步接口均有callback和Promise两种返回形式，下表均以callback形式为例，更多接口及使用方式请见[分布式数据对象](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-data-distributedobject)。
...
2.3 从want.parameters中获取发起端放入的sessionId，调用setSessionId接口设置同步的sessionId。
  ...
  * 跨端迁移需要配置continuable标签，详见[应用接续开发步骤](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-continuation-guide#section1881084093211)。
