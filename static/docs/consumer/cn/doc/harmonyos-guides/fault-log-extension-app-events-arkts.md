...
从API version 21开始，FaultLogExtensionAbility提供了事件订阅接口，用于应用故障事件（包括[崩溃](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cppcrash-guidelines)和[冻屏](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/appfreeze-guidelines)事件）的延迟通知。当应用无法启动或长时间未启动，可以不依赖应用启动而实现故障的hiAppEvent订阅上报。

FaultLogExtensionAbility仅在应用发生崩溃或冻屏事件，并且应用长时间未被重新启动的情况下，用于补充处理故障事件，不能替代[主进程](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/process-model-stage#基本进程类型)正常启动时进行故障事件处理。
...
#### 约束与限制
  ...
  * FaultLogExtensionAbility被拉起后只有很短的时间完成故障处理，建议处理时间不要超过10秒。超时没有处理完成可以在[onDisconnect](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-hiviewdfx-faultlogextensionability#ondisconnect)中保存状态。
  ...
  * FaultLogExtensionAbility进程中订阅的事件需要在主进程中使用hiAppEvent进行订阅。否则，会出现在FaultLogExtensionAbility进程没有接收到回调事件的问题。

  * FaultLogExtensionAbility进程中仅订阅崩溃、应用冻屏事件，不订阅除这两类外的系统事件。否则，会发生系统事件重复上报的问题。

  * FaultLogExtensionAbility进程和主进程定义的watcher名字不能重复。否则，会发生部分事件丢失的问题。
