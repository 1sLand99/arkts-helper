...
参数名 | 类型 | 必填 | 说明  
---|---|---|---  
type | string | 是 | 监听事件，固定为'error'，videoOutput创建成功后可监听。录像接口调用出现错误时触发该事件并返回对应错误码，比如调用start，[CameraOutput.release](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-cameraoutput#release-1)接口时出现错误返回对应错误信息。  
...
启用/关闭镜像录像。
  ...
  * 启用/关闭录像镜像后，需要通过getVideoRotation获取录像旋转角度以及[updateRotation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder#updaterotation12)更新旋转角度。
...
类型 | 说明  
---|---  
[ImageRotation](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-camera-e#imagerotation) | 获取录像旋转角度。  
