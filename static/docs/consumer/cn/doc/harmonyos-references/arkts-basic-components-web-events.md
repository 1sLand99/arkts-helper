...
只要网页进程一直无响应，此回调仍可能会持续触发，直到网页进程再次响应，此时onRenderProcessResponding将会触发。
...
参数名 | 类型 | 必填 | 说明  
---|---|---|---  
callback | Callback<[OnScrollEvent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-basic-components-web-i#onscrollevent12)> | 是 | 当页面滑动到指定位置时触发。  
...
当Web页面触发window.open(url, name)时，会根据name查找是否存在已绑定的Web实例。若存在，该实例将收到此回调以通知应用需将其展示至前端；若不存在，则通过onWindowNew通知应用创建新Web实例。



  * 通过name绑定Web实例‌：需在onWindowNew回调中调用event.handler.setWebController方法，并传入新Web实例的controller，以完成绑定。
...
通知用户窗口关闭请求。和onWindowNew一样，从安全角度讲，应用应该确保用户可以知道他们交互的页面已关闭。
...
网站安全风险检查结束时触发的回调，使用callback异步回调。
...
参数名 | 类型 | 必填 | 说明  
---|---|---|---  
callback | [OnSafeBrowsingCheckResultCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-basic-components-web-t#onsafebrowsingcheckresultcallback11) | 是 | 回调函数，返回收到网站安全风险检查结果。  
...
此外，该功能需通过调用[setErrorPageEnabled](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-webview-webviewcontroller#seterrorpageenabled20)接口启用默认错误页后，才会生效。
...
API version 8开始支持，从API version 10开始废弃，建议使用onLoadIntercept10+代替。
...
从API version 8开始支持，从API version 9开始废弃，建议使用onRenderExited9+代替。
