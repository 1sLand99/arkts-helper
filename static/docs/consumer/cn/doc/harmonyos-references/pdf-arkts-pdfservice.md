...
对PDF文件进行编辑操作后，需要调用saveDocument接口将PDF文件保存，确保编辑操作生效。
...
以下错误码的详细介绍请参见[错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/pdf-error-code)。
...
插入PDF文档页眉页脚。该方法属于耗时业务，需要遍历每一页去添加页眉页脚，添加页面较多时建议放到线程里去处理。
...
插入PDF文档水印。该方法属于耗时业务，需要遍历每一页去添加水印，添加页面较多时建议放到线程里去处理。
...
插入PDF文档背景。该方法属于耗时业务，需要遍历每一页去添加背景，添加页面较多时建议放到线程里去处理。
...
contextString | boolean | 否 | 否 | 搜索命中项的上下文字符串（不支持输出跨页的上下文字符串），当字符串长度不超过contextStringLength时，按原字符串进行输出。  
...
PDF页面的文本批注类，继承PdfAnnotationInfo。
...
PDF页面的链接类型注释的信息，继承PdfAnnotationInfo。
...
PDF页面的自由文本类型注释的信息，继承PdfAnnotationInfo。
...
PDF页面的方块类型标注信息类，继承PdfAnnotationInfo。
...
PDF页面的椭圆型标注的信息类，继承PdfAnnotationInfo。
...
PDF页面的多边形批注信息类，继承PdfAnnotationInfo。
...
PDF页面的线型标注信息类，继承PdfAnnotationInfo。
...
PDF页面的折线类型标注的信息类，继承PdfAnnotationInfo。
...
PDF页面的高亮类型标注信息类，继承PdfAnnotationInfo。
...
PDF页面的下划线类型标注的信息类，继承PdfAnnotationInfo。
...
PDF页面的删除线类型批注的信息类，继承PdfAnnotationInfo。
...
PDF页面的墨水类型注释信息类，继承PdfAnnotationInfo。
...
PDF页面的图章类型注释的信息类，继承PdfAnnotationInfo。
...
getAreaPixelMapWithOptions(matrix:PdfMatrix,bitmapwidth:mumber,bitmapHeight:number,options?:PixelOptions):image.PixelMap
