...
获取已加载的组件的截图，传入组件的[uniqueId](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-framenode#getuniqueid12)，找到对应组件进行截图。使用Promise异步回调。
...
参数名 | 类型 | 必填 | 说明  
---|---|---|---  
uniqueId | number | 是 |  目标组件的[uniqueId](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-framenode#getuniqueid12) **说明：** 不支持未挂树组件，当传入的组件标识是离屏或缓存未挂树的节点时，系统不会对其进行截图。  
options | [componentSnapshot.SnapshotOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-componentsnapshot#snapshotoptions12) | 否 | 截图相关的自定义参数。  
...
获取已加载的组件的截图，传入组件的[uniqueId](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-framenode#getuniqueid12)，找到对应组件进行截图。同步等待截图完成返回[PixelMap](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-pixelmap)。
