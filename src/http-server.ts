...
const PORT = parseInt(process.env.ARKTS_MCP_PORT || '3456', 10);
...
export function startHttpServer(searcher: DocsSearcher): void {
  ...
  // MCP-style SSE endpoint for streaming (optional future enhancement)
  ...
  app.listen(PORT, () => {
    console.error(`[arkts-mcp] HTTP server listening on port ${PORT}`);
    console.error(`[arkts-mcp] Endpoints:`);
